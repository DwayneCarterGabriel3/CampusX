...
import { 
  ...
  Image as ImageIcon, Video, Flame, 
...
export default function Dashboard() {
  ...
  // Carousel State for the sliding campus gists
  ...
  async function handlePublish() {
    ...
    // UPLOAD LOGIC: This sends the file to Supabase Storage
    if (mediaFile) {
      const fileName = `${Date.now()}-${mediaFile.name}`;
      const { data, error } = await supabase.storage
        .from('post-media') // Make sure you created this bucket!
    ...
    const { error } = await supabase.from('posts').insert([{ 
      ...
      image_url: mediaUrl, // Saves the link to the picture/video
      type: 'amebo' 
    }]);
    ...
    <div className="min-h-screen bg-[#050a14] text-white">
      ...
      <nav className="flex items-center justify-between px-8 py-4 border-b border-white/5 sticky top-0 bg-[#050a14] z-50">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center bg-white/5 rounded-2xl px-4 py-2 w-64 border border-white/10">
        ...
        <div className="flex items-center gap-10">
          ...
          <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=John" className="w-9 h-9 rounded-full border-2 border-orange-500" />
        </div>
      </nav>
      ...
      <main className="max-w-6xl mx-auto p-10 grid grid-cols-12 gap-10">
          ...
          <div className="h-[200px] rounded-[2.5rem] overflow-hidden relative shadow-2xl">
            {campusGists.map((gist, index) => (
              <div key={index} className={`absolute inset-0 p-10 transition-all duration-1000 ${gist.color} ${index === currentSlide ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full'}`}>
                <p className="text-[10px] font-black uppercase opacity-60 mb-2">Hottest Amebo</p>
        ...
        {/* SIDEBAR */}
        <div className="col-span-4 bg-white/5 border border-white/10 p-8 rounded-[2.5rem] h-fit">
          <h3 className="text-xs font-black uppercase text-orange-500 mb-6 italic">Who's Online</h3>
          {[1, 2, 3].map(u => (
            <div key={u} className="flex items-center gap-4 mb-4">
              <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${u}`} className="w-10 h-10 rounded-xl" />
              <p className="text-xs font-bold uppercase">Active Student {u}</p>
            </div>
          ))}
        </div>
      </main>
